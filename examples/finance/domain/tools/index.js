...
const WEB_URL = process.env.FINANCE_WEB_URL || 'http://localhost:5180';
const STORE_PATH = process.env.FINANCE_SESSION_STORE || path.resolve(process.cwd(), '.finance-sessions.json');

function readStoreSync() {
  try { const t = fsSync.readFileSync(STORE_PATH, 'utf8'); return JSON.parse(t); } catch { return { active: null, byName: {} }; }
}
...
function postJSONSync(url, body) {
  // Use curl synchronously to avoid async in Liu plans
...
export function financeEnsureSession({ name, title, serverUrl }) {
  ...
  if (store.byName[name]) {
    ...
    return { ok: true, name, sessionId: store.byName[name].sessionId, serverUrl: store.byName[name].serverUrl, reused: true };
  }
...
export function uiAppendTextEnv() {
  const title = process.env.FIN_TEXT_TITLE || '';
  const text = process.env.FIN_TEXT || '';
  const map = getActiveMappingSync(null);
  postJSONSync(`${ensureUrl(map.serverUrl)}/api/append`, { sessionId: map.sessionId, block: { kind: 'text', title, text } });
  return { ok: true };
}

export function uiAppendRequestEnv() {
  const text = process.env.FIN_REQ_TEXT || '';
  const map = getActiveMappingSync(null);
  postJSONSync(`${ensureUrl(map.serverUrl)}/api/append`, { sessionId: map.sessionId, block: { kind: 'request', text } });
  return { ok: true };
}
...
function getActiveMappingSync(explicitSessionId) {
  ...
  const store = readStoreSync();
  const name = store.active;
  if (!name) throw new Error('No active session. Call financeEnsureSession or financeUseSession first.');
...
export function uiAppendRequest({ sessionId, text }) {
  const map = getActiveMappingSync(sessionId);
  postJSONSync(`${ensureUrl(map.serverUrl)}/api/append`, { sessionId: map.sessionId, block: { kind: 'request', text: String(text || '') } });
  return { ok: true };
}

export function uiAppendText({ sessionId, title, text }) {
  const map = getActiveMappingSync(sessionId);
  postJSONSync(`${ensureUrl(map.serverUrl)}/api/append`, { sessionId: map.sessionId, block: { kind: 'text', title: title || '', text: String(text || '') } });
  return { ok: true };
}

export function uiAppendChart({ sessionId, spec, data }) {
  const map = getActiveMappingSync(sessionId);
  postJSONSync(`${ensureUrl(map.serverUrl)}/api/append`, { sessionId: map.sessionId, block: { kind: 'chart', spec: spec || {}, data: Array.isArray(data) ? data : [] } });
  return { ok: true };
}

export function fetchDailyOHLC({ symbol = 'AAPL', period = '1M' }) {
  // Offline dataset: examples/finance/domain/data/aapl_ohlc_month.json
  const dataPath = path.join(__dirname, '..', 'data', 'aapl_ohlc_month.json');
  const text = fsSync.readFileSync(dataPath, 'utf8');
